...
GHBU_BACKUP_DIR=${GHBU_BACKUP_DIR-"gitback"}                  # where to place the backup files
GHBU_ORG=${GHBU_ORG-"m4team"}                                   # the GitHub organization whose repos will be backed up
                                                                     # (if you're backing up a user's repos instead, this should be your GitHub username)
GHBU_UNAME=${GHBU_UNAME-"iranonandon"}                               # the username of a GitHub account (to use with the GitHub API)
GHBU_PASSWD=${GHBU_PASSWD-"1godzilla"}                             # the password for that account 
...
zip -r gitback-${TSTAMP}.zip gitback
